...
<div class="main">
  <div id="search" class="dp100 mt1">
  	<form id="amUserRoleForm" action="<c:url value='/servlet/amUserRole?action=toList'/>" method="post" OnSubmit="addQueryFilterEl(this)">
  	<input type="hidden" name="queryFilters" value="<c:out value='${amUserRole.queryFilters}'/>"/>
  ...
  <div id="content" class="dp100 mt1">
    <table id="amUserRoleTable" class="table">
      <caption><fmt:message key="amUserRole.list.tile"/></caption>
	  <thead>
	    <tr>
		  <td colspan="3">
		...
		<tr>
		  ...
		  <th style="width:50%"><fmt:message key="amUserRole.fieldName1"/></th>
		  <th style="width:49%"><fmt:message key="amUserRole.fieldName2"/></th>
		</tr>
	  </thead>
	  <tbody class="list">
	    <c:forEach var="model" items="${page.result}">
		  <tr>
		    <td><input name="pk" type="checkbox" value="<c:out value='${model.userRoleId}'/>"/></td>
		    <td>
			  <a href="<c:url value='/servlet/amUserRole?action=toView&userRoleId=${model.userRoleId}'/>">
	  ...
	  <tfoot>
	    <tr><td colspan="3"><script type="text/javascript">var form = "amUserRoleForm";</script>
...
function view() {
	...
	if (Jp.isEdit(checkValues)) {
		window.location.href="<c:url value='/servlet/amUserRole?action=toView&userRoleId='/>"+checkValues;	
	}
}
function add() {
	window.location.href="<c:url value='/servlet/amUserRole?action=toAdd'/>";
}
function edit() {
	var checkValues = Jp.getCheckValues("pk");
	if (Jp.isEdit(checkValues)) {
		window.location.href="<c:url value='/servlet/amUserRole?action=toEdit&userRoleId='/>"+checkValues;	
	}
}
function reset() {
	window.location.href="<c:url value='/servlet/amUserRole?action=toList'/>";
}
...
function batchDelete() {
	...
	formEl.action = "<c:url value='/servlet/amUserRole?action=delete'/>";
	formEl.submit();
}
