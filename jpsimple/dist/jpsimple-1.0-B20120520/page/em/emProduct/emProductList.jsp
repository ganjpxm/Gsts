    ...
    <table id="searchTable" class="table">
	  ...
	  <tbody class="formEdit">
	    <tr>
	      ...
	      <th><fmt:message key="emProduct.productName"/></th>
	  	  <td><input id="productName" name="productName" value="<c:out value='${productName}'/>"/></td>
		  <th><fmt:message key="emProduct.price"/></th>
		  <td><input id="price" name="price" value="<c:out value='${price}'/>"/></td>
	    </tr>
	  </tbody>
	  <tfoot>
        <tr>
    ...
    <table id="emProductTable" class="table">
	  ...
	  <thead>
		...
		<tr>
		  ...
		  <th style="width:24%"><fmt:message key="emProduct.productName"/></th>
		  <th style="width:24%"><fmt:message key="emProduct.price"/></th>
		  <th style="width:24%"><fmt:message key="emProduct.publishName"/></th>
		  <th style="width:24%"><fmt:message key="emProduct.publishDate"/></th>
		</tr>
	  </thead>
	  <tbody class="list">
		  ...
		  <tr>
		    ...
		    <td>
			  <a href="<c:url value='/servlet/emProduct?action=toView&productId=${model.productId}'/>">
			    <c:out value="${model.productName}"/>
			  </a>
		    </td>
		    <td><c:out value='${model.price}'/></td>
		    <td><c:out value='${model.publishName}'/></td>
		    <td><c:out value='${model.publishDate}'/></td>
          </tr>
	    </c:forEach>
	  </tbody>
	  <tfoot>
	    <tr><td colspan="5"><script type="text/javascript">var form = "emProductForm";</script>
