...
import org.ganjp.core.util.resource.PathMatchingResourcePatternResolver;
import org.ganjp.core.util.resource.Resource;
import org.ganjp.core.util.resource.ResourcePatternResolver;
...
public abstract class ResourceUtil {
	...
	protected static Logger log = LoggerFactory.getLogger(ResourceUtil.class);
   ...
   // ResourceBundle resource = ResourceBundle.getBundle("i18n/messages", Locale.SIMPLIFIED_CHINESE);
	
	/**
	 * 已知在websphere6.1下有bug，不能正确加载类似path="classpath*:*mvc.xml"这样的文件。
	 * 所以这里限定，如果path的值类似与"classpath*:*.mvc.xml"，会抛出Runtime异常。
	 * 如果想使用classpath*:，在要加载的文件前必须有文件夹，比如"classpath*:config/*.mvc.xml"
	 * 
	 * @param path
	 * @return
	 */
	public static Resource[] loadResourceFromClassPath(String path) {
		try {
