...
package org.ganjp.core.util.resource;
...
import org.ganjp.core.util.ResourceUtil;
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	protected Resource resolveRootDirResource(Resource original) throws IOException {
		if (equinoxResolveMethod != null) {
			...
			if (url.getProtocol().startsWith("bundle")) {
				return new UrlResource((URL) ReflectUtil.invokeMethod(equinoxResolveMethod, null, new Object[] {url}));
			}
		}
		return original;
	}
	 ...
	 * @see #doFindPathMatchingJarResources
	 * @see org.springframework.util.ResourceUtil#isJarURL
	 */
	protected boolean isJarResource(Resource resource) throws IOException {
		return ResourceUtil.isJarURL(resource.getURL());
	}
	...
	protected Set doFindPathMatchingJarResources(Resource rootDirResource, String subPattern) throws IOException {
		...
		if (con instanceof JarURLConnection) {
			...
			int separatorIndex = urlFile.indexOf(ResourceUtil.JAR_URL_SEPARATOR);
			if (separatorIndex != -1) {
				jarFileUrl = urlFile.substring(0, separatorIndex);
				rootEntryPath = urlFile.substring(separatorIndex + ResourceUtil.JAR_URL_SEPARATOR.length());
	...
	protected JarFile getJarFile(String jarFileUrl) throws IOException {
		if (jarFileUrl.startsWith(ResourceUtil.FILE_URL_PREFIX)) {
			try {
				...
				return new JarFile(jarFileUrl.substring(ResourceUtil.FILE_URL_PREFIX.length()));
			}
		}
		else {
	...
	protected Set retrieveMatchingFiles(File rootDir, String pattern) throws IOException {
		...
		String fullPattern = StringUtil.replace(rootDir.getAbsolutePath(), File.separator, "/");
		if (!pattern.startsWith("/")) {
			fullPattern += "/";
		}
		fullPattern = fullPattern + StringUtil.replace(pattern, File.separator, "/");
		Set result = new LinkedHashSet(8);
	...
	protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set result) throws IOException {
		...
		for (int i = 0; i < dirContents.length; i++) {
			...
			String currPath = StringUtil.replace(content.getAbsolutePath(), File.separator, "/");
