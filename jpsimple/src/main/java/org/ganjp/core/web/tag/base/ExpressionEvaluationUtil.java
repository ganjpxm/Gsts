 ...
 */
public abstract class ExpressionEvaluationUtil {
	...
	private static final String EXPRESSION_CACHE_FLAG_CONTEXT_ATTR =
			ExpressionEvaluationUtil.class.getName() + ".CACHE_JSP_EXPRESSIONS";

	private static final String EXPRESSION_CACHE_MAP_CONTEXT_ATTR =
			ExpressionEvaluationUtil.class.getName() + ".JSP_EXPRESSION_CACHE";
	...
	private static final Logger logger = LoggerFactory.getLogger(ExpressionEvaluationUtil.class);

	private static ExpressionEvaluationHelper helper;


	static {
		ClassLoader cl = ExpressionEvaluationUtil.class.getClassLoader();
		if (ClassUtil.isPresent(JSP_20_CLASS_NAME, cl)) {
			logger.debug("Found JSP 2.0 ExpressionEvaluator");
			if (ClassUtil.isPresent(JAKARTA_JSTL_CLASS_NAME, cl)) {
		...
		else if (ClassUtil.isPresent(JAKARTA_JSTL_CLASS_NAME, cl)) {
			logger.debug("Found Jakarta JSTL ExpressionEvaluatorManager");
