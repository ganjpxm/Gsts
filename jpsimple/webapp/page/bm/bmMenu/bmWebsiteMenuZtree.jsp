...
<html>
<head>
  <title><fmt:message key="bmMenu.website.tree.title"/></title>
...
<div class="main">
  <div id="body" class="dp100">
	  <div id="left" class="dl frameborder">
	    <div id="treetitle" class="barTitle"><fmt:message key="bmMenu.website.tree.edit"/></div>
		<div class="buttonBar">
  ...
  <div id="menu" style="position:absolute; display:none; background: #f9f9f9;">
    <table>
	  <tr><td><a href="javascript:addNodes(true);"><fmt:message key="bmMenu.website.tree.sub.node.add"/></a></td></tr>
	  <tr><td><a href="javascript:addNodes(false);"><fmt:message key="bmMenu.website.tree.end.node.add"/></a></td></tr>
	  <tr><td><a href="javascript:editMenu();"><fmt:message key="bmMenu.website.tree.node.edit"/></a></td></tr>
	  <tr><td><a href="javascript:delNode();"><fmt:message key="bmMenu.website.tree.node.delete"/></a></td></tr>
	</table>
  </div>
</div>
</body>
...
var treeConfig = {
  ...
  keepLeaf: true,
  async: true,
  asyncUrl: "<c:url value='/servlet/bmMenu?action=getWebsiteZtreeJsonNodes'/>",
  asyncParam: ["id","pids"],
  asyncParamOther: ["isAllLoad",false],
  root: {isRoot: true,nodes: [{id:'00000000000000000000000000000002', pid:'0', pids:'', cd:'02', name:'website menu category', isParent:true}]},
  nameCol: "name",
  callback: {
...
function onClick(event, treeName, treeNode) {
  ...
  if (!Jp.isEmpty(pids)) {
	...
	if (pids=="0") articleCategoryIds = treeNode.id;
	var themeVocabularyIndex = pids.indexOf("4028380436614083013661448cf80007")==-1?pids.indexOf("40283804366c388001366c3880e50000"):1;
	var url = "";
	if ( themeVocabularyIndex!= -1) {//theme vocabulary
      url = "<c:url value='/servlet/cmVocabulary?action=toList&vocabularyCatagoryIds='/>" + pids.substring(themeVocabularyIndex) 
    		  + "," + treeNode.id;
    } else if (!treeNode.isParent) {
	  url = "<c:url value='/servlet/bmMenu?action=toEdit&menuId='/>" + treeNode.id;
    } else {
	  articleCategoryIds = pids + "," + treeNode.id;
	  url = "<c:url value='/servlet/cmArticle?action=toList&articleCategoryIds='/>" + articleCategoryIds;
    }
...
function editMenu() {
  ...
  if (selectedNode.id!="01") {
	  document.getElementById("iframeContent").src = "<c:url value='/servlet/bmMenu?action=toEdit&menuId='/>" + selectedNode.id;	  
  }
}
