...
<div class="main">
  <div id="search" class="dp100 mt1">
  	<form id="cmVocabularyForm" action="<c:url value='/servlet/cmVocabulary?action=toList'/>" method="post" OnSubmit="addQueryFilterEl(this)">
  	<input type="hidden" name="queryFilters" value="<c:out value='${cmVocabulary.queryFilters}'/>"/>
	  ...
	  <tbody class="formEdit">
	    <tr>
	      ...
	      <th><fmt:message key="cmVocabulary.foreignLanguage"/></th>
	  	  <td><input id="foreignLanguage" name="foreignLanguage" value="<c:out value='${foreignLanguage}'/>"/></td>
		  <th><fmt:message key="cmVocabulary.chinese"/></th>
		  <td><input id="chinese" name="chinese" value="<c:out value='${chinese}'/>"/></td>
	    </tr>
	  </tbody>
	  <tfoot>
        <tr>
  ...
  <div id="content" class="dp100 mt1">
    <table id="cmVocabularyTable" class="table">
      <caption><fmt:message key="cmVocabulary.list.tile"/></caption>
	  <thead>
	    <tr>
		  <td colspan="4">
		...
		<tr>
		  ...
		  <th style="width:33%"><fmt:message key="cmVocabulary.foreignLanguage"/></th>
		  <th style="width:33%"><fmt:message key="cmVocabulary.phonogram"/></th>
		  <th style="width:33%"><fmt:message key="cmVocabulary.chinese"/></th>
		</tr>
	  </thead>
	  <tbody class="list">
	    <c:forEach var="model" items="${page.result}">
		  <tr>
		    <td><input name="pk" type="checkbox" value="<c:out value='${model.vocabularyId}'/>"/></td>
		    <td>
			  <a href="<c:url value='/servlet/cmVocabulary?action=toView&vocabularyId=${model.vocabularyId}'/>">
			    <c:out value="${model.foreignLanguage}"/>
			  </a>
		    </td>
		    <td><c:out value='${model.phonogram}'/></td>
		    <td><c:out value='${model.chinese}'/></td>
          </tr>
	    </c:forEach>
	  </tbody>
	  <tfoot>
	    <tr><td colspan="4"><script type="text/javascript">var form = "cmVocabularyForm";</script>
...
function view() {
	...
	if (Jp.isEdit(checkValues)) {
		window.location.href="<c:url value='/servlet/cmVocabulary?action=toView&vocabularyId='/>"+checkValues;	
	}
}
function add() {
	window.location.href="<c:url value='/servlet/cmVocabulary?action=toAdd&vocabularyCatagoryIds=${vocabularyCatagoryIds}'/>";
}
function edit() {
	var checkValues = Jp.getCheckValues("pk");
	if (Jp.isEdit(checkValues)) {
		window.location.href="<c:url value='/servlet/cmVocabulary?action=toEdit&vocabularyId='/>"+checkValues;	
	}
}
function reset() {
	window.location.href="<c:url value='/servlet/cmVocabulary?action=toList'/>";
}
...
function batchDelete() {
	...
	formEl.action = "<c:url value='/servlet/cmVocabulary?action=delete'/>";
	formEl.submit();
}
